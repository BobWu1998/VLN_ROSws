...
class MapPredictorHier(nn.Module):
    ...
    def forward(self, batch, is_train=True):
        ...
        B, T, spatial_C, cH, cW = step_ego_crops.shape # batch, sequence length, _, crop height, crop width
        objects_C = batch['pred_ego_crops_sseg'].shape[2]

        step_ego_crops = step_ego_crops.view(B*T, spatial_C, cH, cW)
        pred_ego_crops_sseg = batch['pred_ego_crops_sseg'].view(B*T, objects_C, cH, cW)

        if self.with_img_segm:
            pred_maps_raw_spatial, pred_maps_raw_objects = self._segmentation_model(step_ego_crops, pred_ego_crops_sseg)
        else:

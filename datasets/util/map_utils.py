...
def ground_projection(points2D, local3D, sseg, sseg_labels, grid_dim, cell_size):
    ...
    for i in range(sseg.shape[0]): # sequence length
        ...
        ego_grid_sseg[i,:,:,:] = grid_sseg #[1, 27, 192, 192]
...
def label_pooling(sseg, points2D, map_coords, sseg_labels, grid_dim):
    ...
    for i in range(uniq_rows.shape[0]):
        ...
        hist = torch.histc(bin_lbls.float(), bins=sseg_labels, min=0, max=sseg_labels)
...
def get_gt_map(x, y, label_seq, abs_pose, grid_dim, cell_size, color_pcloud=None, z=None):
    ...
    # ** In order to cover empty spots in the generated gt maps when cell_size<0.1m :
    # ** Find the missing coordinates from map_coords (differentiate between areas outside the map using a distance threshold)
    # ** Select the nearest neighbour to decide on label and rgb value
...
def update_sseg_with_occ(pred_ego_crops_sseg, ego_grid_sseg_3, crop_size):
    ...
    ego_grid_map = crop_grid(grid=ego_grid_sseg_3, crop_size=crop_size)
    ego_grid_map = ego_grid_map.unsqueeze(0) # [1, 1, 3, 192, 192]
    ego_unocc_map = ego_grid_map.squeeze(0).squeeze(0).cpu().detach().numpy() # [3, 192, 192]

    pred_ego_crops_sseg = pred_ego_crops_sseg.squeeze(0).squeeze(0).cpu().detach().numpy() # [27, 192, 192]
    # print(ego_unocc_map.shape, pred_ego_crops_sseg.shape)
    ...
    # pred_ego_crops_sseg[17,floor_idx] = ego_unocc_map[2,floor_idx] #1.0 # floor
    # pred_ego_crops_sseg[:17,floor_idx] = (1.0 - pred_ego_crops_sseg[17,floor_idx])/26
    # pred_ego_crops_sseg[18:,floor_idx] = (1.0 - pred_ego_crops_sseg[17,floor_idx])/26


    # viz_utils.vis_arr(pred_ego_crops_sseg[16],'/','other_val')

    pred_ego_crops_sseg = torch.tensor(pred_ego_crops_sseg, device='cuda')
    pred_ego_crops_sseg = pred_ego_crops_sseg.unsqueeze(0).unsqueeze(0)
...
# def sseg_floor_update(ego_grid_map, pred_ego_crops_sseg)